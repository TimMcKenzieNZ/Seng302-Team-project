...
public class newAdminMainMenuController {
    ...
    @FXML
    private Button viewDonorsButton;
    @FXML
    private Button viewCliniciansButton;
    @FXML
    ...
    @FXML
    private Button viewEditAdmin;


    @FXML
    public void openTransplantWaitingList(){
...
//                    informationLabel.setText(""); // we clear the edit label in the admin profile.
    ...
    protected void getAdmin(boolean setup){
        String endpoint = ADDRESSES.GET_ADMIN.getAddress() + App.getCurrentSession().getUsername();
        GetTask task = new GetTask(Administrator.class, ADDRESSES.SERVER.getAddress(), endpoint, App.getCurrentSession().getToken());
        task.setOnSucceeded(event -> {
            ...
            if(admin.getUserName().equals("Sudo")) {
                viewEditAdmin.setVisible(false);
            }
    ...
    public void viewAdmin() throws IOException {
        ...
        GetTask task = new GetTask(Administrator.class, ADDRESSES.SERVER.getAddress(), ADDRESSES.GET_ADMIN.getAddress() + admin.getUserName(), App.getCurrentSession().getToken());
        task.setOnSucceeded(event -> {
    ...
    public void loadProfileWindow(Administrator admin) throws IOException{
        ...
        if (!App.childWindowToFront(admin)) {
            ...
            AdministratorViewProfileController.setSelectedAdmin(admin);

            // Create new pane.
            FXMLLoader loader = new FXMLLoader();
            StackPane profilePane = loader.load(getClass().getResourceAsStream(PageNav.VIEWADMIN));
            ...
            App.addChildWindow(profileStage, admin);
        }
    }
